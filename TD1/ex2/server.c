...
static bool  sousProcessusExiste = false;
...
void sousProcessusCompeleteHandler(){
	sousProcessusExiste = false;
}

void traiteMessage(msg *message, int *nbClient, objet *clientPanier, int *num_client, bool isSousProcessus){
	char tmpLog[255];
	switch(message->req)
	{
		case DEM_NUM_CLT:
...
int main()
{
	...
	msg message;
	//Pour savoir si le serveur est en train de traiter le meme client en 2 ligne
	bool clientTable[NB_MAX_CLT + 1] = {false, false, false, false, false};
	...
	else
	{
		...
		else
		{
			while(1){
				...
				if (!clientTable[message.num_clt])
				{
					clientTable[message.num_clt] = true;
				}else{
					message.type = message.num_clt;
				...
				if(!sousProcessusExiste){
					...
					if(sp == 0){
						msgrcv(id_msg, &message, MSG_SIZE, REQ_SRV, 0);
						puts("Sub Task:");
						traiteMessage(&message, &nb_client, clientPanier, &num_client, true);
						puts("Sub over.");	
						if (message.type == REP_NUM_CLT)
						{
							clientTable[0] = false;
						} else{
							clientTable[message.num_clt] = false;
						}
						return 0;
					}
				}

				sousProcessusExiste = true;
				puts("Main task:");
				traiteMessage(&message, &nb_client, clientPanier, &num_client, false);
				if (message.type == REP_NUM_CLT)
				{
					clientTable[0] = false;
				} else{
